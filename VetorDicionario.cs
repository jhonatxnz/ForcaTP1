...
class VetorDicionario
{
  ...
  public void GravarDados(string nomeArquivo)
  {
    ...
    public bool ExisteSemOrdem(string PalavraProcurada, out int ondeEsta)  // pesquisa sequencial em vetor desordenado
    {
        ...
        {
            if (dados[ondeEsta].Palavra.Trim() == PalavraProcurada.Trim())
    ...
    public bool ExisteEmOrdem(string PalavraProcurada, out int onde)  // pesquisa sequencial em vetor ordenado
    {
        bool fim, achou;
        ...
        while (!achou && !fim) // not (achou or fim) // De Morgan
            if (onde >= qtosDados) // condição i
              ...
              if (dados[onde].Palavra == PalavraProcurada)
                achou = true; // condição ii
            else
                if (dados[onde].Palavra.Trim().Length > PalavraProcurada.Trim().Length)
    ...
    public bool Existe(string PalavraProcurada, out int onde)  // onde --> posicao onde achou ou onde deveria estar (inclusão)
    {
        ...
        {
            onde = (inicio + fim) / 2;
            if (PalavraProcurada.Trim().ToUpper() == dados[onde].Palavra.Trim())    //Trim()apaga os espaços a direita ToUpper()deixa a letra em maiscula
                achou = true;  // a posição dessa matrícula no vetor é o índice "onde"
            else
              if (PalavraProcurada.Length < dados[onde].Palavra.Length)
                fim = onde - 1;
            else
                inicio = onde + 1;
        }
  ...
  public void OrdenarPorNome(ListSortDirection qualOrdem)
  {
    if (qualOrdem == ListSortDirection.Ascending)   // ordem crescente
    {
      for (int lento = 0; lento < qtosDados; lento++)
        for (int rapido = lento + 1; rapido < qtosDados; rapido++)
          if (dados[lento].Dica.CompareTo(dados[rapido].Dica) > 0) // nomes fora de ordem
            Trocar(lento, rapido);
    }
    else
      for (int lento = 0; lento < qtosDados; lento++)
